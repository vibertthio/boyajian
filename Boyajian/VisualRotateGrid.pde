...
class RotateGrid {
  ...
  void rowStretchSequenceUpdate() {
    ...
    if (rowStretchSequence.getBang()) {
      int index = rowStretchSequence.getSignal();
      rowStretchBang(index, 10);
    }
  }
...
class RotateRectangle {
  ...
  void alphaUpdate() {
    targetAlpha = map(layer[6], 135, 255, 0, 170);
